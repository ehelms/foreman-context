...
Stage only the files relevant to this change — add files individually by name (e.g. `git add path/to/file`) instead of `git add -A` or `git add .`, unless every changed file belongs in this commit. Inspect staged files for secrets before committing.
